            ...
            Puissance4Theme {
                Surface(
                    ...
                    when (currentScreen) {
                        ...
                        "gameHistory" -> GameHistoryScreen(onNavigateBack = {
                            currentScreen = "mainMenu"
                        })
                    }
                }
...
fun PlayerVsAIGameScreen(
    ...
    // Dialogue pour entrer le nom du joueur
    if (showDialog) {
        ...
        Surface(
            ...
            Column(
                ...
                if (winner != 0) {
                    Text(
                        text = if (winner == 1) "$playerName a gagné !" else "L'IA a gagné !",
                        style = MaterialTheme.typography.titleLarge
                    )
                } else {
                    Text(
                        text = if (currentPlayer == 1) "Tour de $playerName" else "Tour de l'IA",
                        style = MaterialTheme.typography.titleLarge
                    )
                }
                ...
                LaunchedEffect(currentPlayer, grid) {
                    if (currentPlayer == 2 && winner == 0) {
                        val (_, bestColumn) = minimaxWithAlphaBeta(
                            grid,
                            difficulty,
                            true,
                            Int.MIN_VALUE,
...
fun PlayerVsPlayerGameScreen(
            ...
            Column(
                ...
                if (winner != 0) {
                    LaunchedEffect(winner) {
                        if (winner == 1) scorePlayer1++ else if (winner == 2) scorePlayer2++
                    }
                    Text("Le joueur ${if (winner == 1) playerName1 else playerName2} a gagné !", style = MaterialTheme.typography.titleLarge)
                    Text("Score actuel - $playerName1 : $scorePlayer1, $playerName2 : $scorePlayer2", style = MaterialTheme.typography.bodyLarge)
                } else {
                    Text("Tour de ${if (currentPlayer == 1) playerName1 else playerName2}", style = MaterialTheme.typography.titleLarge)
                }
                ...
                Button(onClick = {
                    // Logique pour quitter la partie et retourner au menu
                    onGameEnd()
                }) {
                    Text("Quitter la partie")
                }

                // Affichez les boutons "Rejouer" et "Retour au menu" uniquement si une partie est terminée
                if (winner != 0) {
                    Spacer(modifier = Modifier.height(8.dp))
                    Button(onClick = {
                        grid = List(6) { MutableList(7) { 0 } }
                        currentPlayer = 1
                        winner = 0
                        // Ne pas réinitialiser les scores pour permettre la continuation du jeu
                    }) {
                        Text("Rejouer")
                    }
                }
    ...
    fun minimaxWithAlphaBeta(
        ...
        if (checkForWin(grid) != 0 || depth == 0) {
            ...
            return Pair(
                if (checkForWin(grid) == 2) 1 else if (checkForWin(grid) == 1) -1 else 0,
                null
            )
        }
        ...
        if (isMaximizing) {
            ...
            for (column in 0 until grid[0].size) {
                ...
                if (row != null) {
                    ...
                    val score =
                        minimaxWithAlphaBeta(newGrid, depth - 1, false, myAlpha, myBeta).first
                    if (score > bestScore) {
                        bestScore = score
                        bestColumn = column
                    }
            ...
            for (column in 0 until grid[0].size) {
                ...
                if (row != null) {
                    ...
                    val score =
                        minimaxWithAlphaBeta(newGrid, depth - 1, true, myAlpha, myBeta).first
                    if (score < bestScore) {
                        bestScore = score
                        bestColumn = column
                    }
    ...
    fun GameGrid(
        grid: List<MutableList<Int>>,
        currentPlayer: Int,
        onMoveMade: (MutableList<MutableList<Int>>, Int) -> Unit
    ) {
        Column {
            for (row in grid) {
                        ...
                        Box(
                            modifier = Modifier
                                ...
                                .clickable {
                                    ...
                                    val (updatedGrid, winner) = onColumnClick(
                                        grid,
                                        columnIndex,
                                        currentPlayer
                                    )
    ...
    fun saveGameResultHumanVsAI(
        ...
        val currentHistory = sharedPreferences.getString("history", "") ?: ""
        val newEntry =
            "$winnerName contre $loserName${if (difficulty != null) " - Difficulté: $difficulty" else ""}\n"
        editor.putString("history", currentHistory + newEntry)
        editor.apply()
    }
